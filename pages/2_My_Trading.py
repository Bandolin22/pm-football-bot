...
st.caption(
    ...
    "factor and by club. Read-only Data API. This page never places orders. "
...
with factor_tab:
    ...
    factor_df = _money(
        ...
        ["Cost $", "Open value $", "Realized $", "Unrealized $", "PnL $"],
    )
    st.dataframe(factor_df, use_container_width=True, hide_index=True)

with team_tab:
    watch = st.checkbox("Watchlist clubs only", value=True)
    st.caption(
        "Watchlist names use the same aliases as Upcoming (Roma, Napoli, Barca, …). "
        "Turn the filter off to see every parsed soccer side."
    )
                ...
                [
                    {
                        ...
                        "Teams": ", ".join(lot.watch_clubs or lot.teams),
...
with closed_tab:
    ...
    closed_lots = sorted(closed_lots, key=lambda lot: lot.pnl)
